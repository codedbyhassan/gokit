...
func ParseAt(input string,asOf time.Time)(Result,error){
    original:=input;clean:=normalize(input);if clean==""{return Result{OriginalInput:original},interpret.ErrEmptyInput}
    ...
    if looksLikeConversion(clean){parsed,err:=unit.Parse(clean);if err==nil{return Result{Kind:Convert,Value:parsed.Value,OriginalInput:original,Format:"conversion request",Confidence:.99},nil};if strings.HasPrefix(clean,"convert ")||strings.HasPrefix(clean,"please convert "){return Result{OriginalInput:original},err}}
    ...
    if node,err:=expression.ParseQuantityAST(clean);err==nil{if result,evalErr:=expression.EvaluateQuantity(node);evalErr==nil&&result.Unit.Name!=""{return Result{Kind:Calculate,Value:calculator.Result{Value:result.Value,Operation:calculator.Add},OriginalInput:original,Format:"quantity expression",Confidence:.99},nil}}
