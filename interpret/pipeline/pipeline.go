...
type Source string
const ( IntentSource Source = "intent"; RouterSource Source = "router" )

type Result struct { OriginalInput string; Source Source; Kind string; Interpretation any; Value any; Confidence interpret.Confidence; Assumptions []string; Plan plan.Plan }

func Parse(input string)(Result,error){return ParseAt(input,time.Now())}
func ParseAt(input string,asOf time.Time)(Result,error){
	if strings.TrimSpace(input)==""{return Result{OriginalInput:input},interpret.ErrEmptyInput}
	intentResult,err:=intent.ParseAt(input,asOf)
	if err==nil{return fromIntent(intentResult)}
	if !errors.Is(err,interpret.ErrUnrecognizedInput){return Result{OriginalInput:input},err}
	routed,err:=router.Parse(input);if err!=nil{return Result{OriginalInput:input},err};return fromRouter(routed)
}
...
func fromIntent(value intent.Result)(Result,error){
	...
	if operation,ok:=value.Value.(expression.Operation);ok{executed,err:=expression.Calculate(operation.OriginalInput);if err!=nil{return Result{OriginalInput:value.OriginalInput},err};result.Value=executed;result.Plan.Add(plan.Step{Name:"execute",Operation:string(operation.Operator),Input:operation,Output:executed,Confidence:value.Confidence});return result,nil}
	// Intent parsers that already execute a domain operation still expose a
	// distinct execution step in the plan so callers can inspect the pipeline.
	if value.Kind==intent.Calculate{result.Plan.Add(plan.Step{Name:"execute",Operation:"calculate",Input:value.Value,Output:value.Value,Confidence:value.Confidence})}
	return result,nil
}
...
func fromRouter(value router.Result)(Result,error){
	...
	if operation,ok:=value.Value.(expression.Operation);ok{executed,err:=expression.Calculate(operation.OriginalInput);if err!=nil{return Result{OriginalInput:value.OriginalInput},err};result.Value=executed;result.Plan.Add(plan.Step{Name:"execute",Operation:string(operation.Operator),Input:operation,Output:executed,Confidence:value.Confidence})}
	return result,nil
}
