...
// ParseQuantity parses a single measurement such as "5kg" or "five kilograms".
func ParseQuantity(input string)(Quantity,error){clean:=normalize(input);for _,alias:=range units.Aliases(){if !strings.HasSuffix(clean,alias){continue};numberText:=strings.TrimSpace(strings.TrimSuffix(clean,alias));if numberText==""{continue};value,err:=number.Parse(numberText);if err!=nil{continue};u,err:=units.Lookup(alias);if err==nil{return Quantity{Value:value.Value,Unit:u},nil}};return Quantity{},fmt.Errorf("invalid measurement %q",input)}

// ParseConversion parses a conversion request and returns a typed conversion.
func ParseConversion(input string)(Conversion,error){r,err:=Parse(input);if err!=nil{return Conversion{},err};conversion,ok:=r.Value.(Conversion);if !ok{return Conversion{},fmt.Errorf("input is not a conversion: %q",input)};return conversion,nil}
