...
func TestWordQuantities(t *testing.T){
	tests:=[]struct{input string;want float64;symbol string}{
		{"five kilograms",5,"kg"},{"two hundred grams",200,"g"},{"one point five liters",1.5,"L"},{"1.25 kg",1.25,"kg"},{"three miles",3,"mi"},{"twenty five celsius",25,"°C"},
	}
...
func TestWordConversion(t *testing.T){r,err:=Parse("convert five kilometers to miles");if err!=nil{t.Fatal(err)};c:=r.Value.(Conversion);if math.Abs(c.Value-3.106855961)<1e-9{return};t.Fatalf("got %v",c.Value)}
